...
layui.define(['layer', 'table', 'laytpl', 'contextMenu'], function (exports) {
    ...
    /* 合并相同单元格 */
    tableX2.merges = function (tableId, indexs, fields, sort) {
        if (typeof fields === 'boolean') {
            sort = fields;
            fields = undefined;
        }
        var $tb = $('[lay-filter="' + tableId + '"]+.layui-table-view>.layui-table-box>.layui-table-fixed>.layui-table-body>table');
        var $trs = $tb.find('>tbody>tr');
        console.log('[lay-filter="' + tableId + '"]+.layui-table-view>.layui-table-box>.layui-table-fixed>.layui-table-body>table');
        // 循环合并每一列
        ...
        if (sort === undefined || sort) {
            table.on('sort(' + tableId + ')', function () {
                tableX2.merges(tableId, indexs, fields, false);
            });
        }
    ...
    /* 表格行绑定鼠标右键 */
    tableX2.bindCtxMenu = function (tableId, items) {
        var data = table.cache[tableId];
    ...
    /* 导出任意数据 */
    tableX2.exportData = function (param) {
        ...
        if (typeof data === 'string') {
            ...
            option.url = data;
            tableX2.loadUrl(option, function (res) {
                layer.close(loadIndex);
                param.data = res;
                tableX2.exportData(param);
            });
            return;
        }
        // 列参数补全
        ...
        // 获取templet后的数据
        var fullData = tableX2.parseTbData(cols, tableX2.deepClone(data), true);
    ...
    /* 导出表格数据(高级) */
    tableX2.exportDataX = function (param) {
    ...
    /* 后端导出数据拼接参数 */
    tableX2.exportDataBack = function (url, where, method) {
        where || (where = {});
    ...
    /* 渲染表格，前端分页 */
    tableX2.renderFront = function (param) {
        ...
        if (param.url) {  // url方式
            ...
            insTb.reloadUrl = function (p) {
                ...
                // 获取url数据
                tableX2.loadUrl(reParam, function (data) {
                    insTb.reload({url: '', data: data, page: {curr: 1}});
                    tableX2.putTbData(tableId, tableX2.parseTbData(reParam.cols, data));  // 缓存数据
            ...
            insTb.reloadData = function (p) {
                insTb.reload(p);
                tableX2.parseTbData(param.cols, p.data);  // 解析temple列
                tableX2.putTbData(tableId, p.data);
                $('input[' + tbSearchAttr + '="' + tableId + '"]').val('');  // 清空搜索输入框
                window.tbX.cacheSearch[tableId] = undefined;  // 重置搜索结果
            };
            tableX2.putTbData(tableId, tableX2.parseTbData(param.cols, param.data));  // 缓存数据
        }
        tableX2.renderAllTool(insTb);  // 渲染工具组件
        return insTb;
    };

    /* 加载表格数据 */
    tableX2.loadUrl = function (options, callback) {
        // 响应数据的自定义格式
    ...
    /* 解析数据表格templet列 */
    tableX2.parseTbData = function (cols, dataList, overwrite) {
        var templets = [];  // 需要解析的列
        table.eachCols(undefined, function (i, item) {
            if (item.templet) {
    ...
    /* 缓存表格的数据 */
    tableX2.putTbData = function (tableId, dataList) {
        window.tbX.cache[tableId] = dataList;
    };

    /* 获取表格缓存的数据 */
    tableX2.getTbData = function (tableId) {
        var dataList = window.tbX.cache[tableId];
        return tableX2.deepClone(dataList || table.cache[tableId]);
    };

    /* 搜索数据 */
    tableX2.filterData = function (dataList, searchName, searchValue) {
        var newList = [], sfs;
        for (var i = 0; i < dataList.length; i++) {
            var obj = dataList[i];
            if (!sfs) {  // 搜索的字段
            ...
            for (var j = 0; j < sfs.length; j++) {
                if (tableX2.isContains(obj[sfs[j]], searchValue)) {
                    newList.push(obj);
                    break;
                }
            }
    ...
    /* 字符串是否包含 */
    tableX2.isContains = function (str1, str2) {
        str1 || (str1 = '');
        str2 || (str2 = '');
    ...
    /* 渲染所有组件 */
    tableX2.renderAllTool = function (insTb) {
    ...
    /* 深度克隆对象 */
    tableX2.deepClone = function (obj) {
        var result;
        var oClass = tableX2.isClass(obj);
        if (oClass === 'Object') {
            result = {};
        ...
        for (var key in obj) {
            ...
            var copy = obj[key];
            if (tableX2.isClass(copy) === 'Object') {
                result[key] = arguments.callee(copy);//递归调用
            } else if (tableX2.isClass(copy) === 'Array') {
    ...
    var renderFrontSearch = function (insTb) {
        ...
        $input.off('keydown').on('keydown', function (event) {
            ...
            var dataList = tableX2.getTbData(tableId);
            var newDataList = tableX2.filterData(dataList, searchName, searchValue);
    ...
    var renderFrontSort = function (insTb) {
        ...
        table.on('sort(' + tableId + ')', function (obj) {
            ...
            dataList || (dataList = tableX2.getTbData(tableId));
            if (sortType) {
            ...
            $('.tbx-dropdown-menu').off('click').on('click', '.tbx-dropdown-menu-item', function (event) {
                ...
                if (type === 'check') {
                    ...
                    tableX2.exportData({
                        fileName: insTb.config.title,
                        cols: insTb.config.cols,
                        data: tableX2.getTbData(tableId)
                    });
                }
                if (event !== void 0) {
                    event.preventDefault();
