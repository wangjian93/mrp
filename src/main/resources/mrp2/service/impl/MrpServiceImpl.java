...
public class MrpServiceImpl implements MrpService {
    ...
    @Autowired
    public MrpServiceImpl(DpsService dpsService, MrpVerRepository mrpVerRepository, BomService bomService,
...
//        mrpVer.setStatus(MrpVer.STATUS_WAIT);
    ...
    public void computeDemand(String dpsVer, String product) {
        ...
        String plant = dpsService.getPlantByDpsVer(dpsVer);
        List<Bom> bomList = bomService.getProductBom(product, plant);
...
//                demand.setMaterialName(bom.getMaterialName());
                ...
                double substituteRate = materialSubstituteService.getMaterialSubstituteRate(bom.getPlant(), bom.getProduct(), bom.getMaterial());
                demand.setSubstituteRate(substituteRate);
                // LCM ： DPS需求量 * Bon使用量 * 替代比列
                // CELL ： DPS需求量 * 1000 * 切片数 * Bon使用量 * 替代比列
                //  需求量不保留小数，向上取整
                double demandQty = 0;
                if(StringUtils.equalsIgnoreCase(dps.getFab(), "CELL")) {
                    demand.setSlice(slice);
                    demandQty = DoubleUtil.multiply(dpsQty , usageQty, slice, 1000d, substituteRate);
                } else if(StringUtils.equalsAnyIgnoreCase(dps.getFab(), "LCM1", "LCM2")) {
                    demandQty = DoubleUtil.multiply(dpsQty , usageQty, substituteRate);
                }
...
//        }
    ...
    private void generateMrpData(String mrpVer, String material, Map<String, List<Demand>> demandMap,
        ...
        mrpDataMaster.setMrpVer(mrpVer);
        mrpDataMaster.setMaterialName(materialNameMap.get(material));
        mrpDataMaster.setMaterialGroup(materialGroupMap.get(material));
        mrpDataMaster.setLossRate(materialLossRateService.getMaterialLossRate(material));
        // 获取期初库存
        mrpDataMaster.setGoodInventory(goodInventoryMap.get(material)==null ? 0 : goodInventoryMap.get(material));
        mrpDataMaster.setDullInventory(dullInventoryMap.get(material)==null  ? 0 : dullInventoryMap.get(material));
        ...
        for(Demand demand : demandList) {
            ...
            if(StringUtils.isEmpty(mrpDataMaster.getProducts())) {
                mrpDataMaster.setProducts(demand.getProduct());
            } else {
                if(!StringUtils.containsIgnoreCase(mrpDataMaster.getProducts(), demand.getProduct())) {
                    mrpDataMaster.setProducts(mrpDataMaster.getProducts() + "/" + demand.getProduct());
                }
            }
            if(StringUtils.isEmpty(mrpDataMaster.getPlant())) {
//                mrpDataMaster.setPlant(demand.getFab());
            } else {
//                if(!StringUtils.containsIgnoreCase(mrpDataMaster.getPlant(), demand.getFab())) {
//                    mrpDataMaster.setPlant(mrpDataMaster.getPlant() + "/" + demand.getFab());
//                }
            }
        }
    ...
    public void generateMrpData(String mrpVer, String material) {
        ...
        // 创建MrpMaterial
        MrpDataMaster mrpMaterial = new MrpDataMaster();
        mrpMaterial.setMaterial(material);
        mrpMaterial.setMrpVer(mrpVer);
        mrpMaterial.setMaterialName(materialService.getMaterialName(material));
        mrpMaterial.setMaterialGroup(materialService.getMaterialGroup(material));
...
//                }
        ...
        mrpDataMasterService.saveMrpMaterial(mrpMaterial);
        mrpDataRepository.saveAll(mrpDataList);
    }
    ...
    public void computeLossQty(String mrpVer) {
        ...
        List<String> materialList = mrpDataMasterService.getMaterial(mrpVer);
        if(materialList== null || materialList.size()==0) return;
        for(String material : materialList) {
    ...
    public void computeBalance(String mrpVer, String material) {
        ...
        for(int i=0; i<dateList.size(); i++) {
            ...
            if(i==0) {
                ...
                double beginInventoryQty = DoubleUtil.sum(mrpDataMaster.getGoodInventory(), mrpDataMaster.getDullInventory());
