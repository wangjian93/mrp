...
public class MrpService3 {
    ...
    @Autowired
    public MrpService3(DpsService dpsService, MrpVerRepository mrpVerRepository,
     ...
     * @return Date[] 日期区间
     */
    public Date[] checkVer(String[] dpsVers, String[] mpsVers) {
        log.info("DPS&MPS版本检查");
        if(dpsVers == null) dpsVers = new String[0];
        if(mpsVers == null) mpsVers = new String[1];
        if(dpsVers.length==0 && mpsVers.length==0) throw new RuntimeException("DPS/MPS版本不能全为空");
        String plant = null;
        List<Date[]> dateInterval = new ArrayList<>();
        // 检查厂别是否一致
        for(String ver : dpsVers) {
            DpsVer dpsVer = dpsService.getDpsVer(ver);
            if(dpsVer == null) {
                throw new RuntimeException("选择的DPS版本"+ver+"不存在");
            }
            if(plant == null) {
                plant = dpsVer.getFab();
            } else {
                if(!StringUtils.equalsIgnoreCase(dpsVer.getFab(), plant)) throw new RuntimeException("选择的DPS/MPS所属厂别不一致");
            }
            dateInterval.add(new Date[] {dpsVer.getStartDate(),  dpsVer.getEndDate()});
        }
        for(String ver : mpsVers) {
            MpsVer mpsVer = mpsService.getMpsVer(ver);
            if(mpsVer == null) {
                throw new RuntimeException("选择的MPS版本"+ver+"不存在");
            }
            if(plant == null) {
                plant = mpsVer.getFab();
            } else {
                if(!StringUtils.equalsIgnoreCase(mpsVer.getFab(), plant)) throw new RuntimeException("选择的DPS/MPS所属厂别不一致");
            }
            dateInterval.add(new Date[] {mpsVer.getStartDate(),  mpsVer.getEndDate()});
        }
        ...
        for(int i=0; i<dateInterval.size(); i++) {
            Date[] dates1 = dateInterval.get(i);
            for(int j=0; j<dateInterval.size(); j++) {
                if(i==j) continue;
                Date[] dates2 = dateInterval.get(j);
                if( ( dates1[0].before(dates2[0]) && dates1[0].before(dates2[1]) && dates1[1].before(dates2[0]) && dates1[1].before(dates2[0]) )
                        || ( dates1[0].after(dates2[0]) && dates1[0].after(dates2[1]) && dates1[1].after(dates2[0]) && dates1[1].after(dates2[0])  ) ) {
                } else {
                    throw new RuntimeException("选择的DPS/MPS日期区间重叠");
                }
            }

            if(startDate == null || startDate.after(dates1[0])) {
                startDate =  dates1[0];
            }
            if(endDate == null || endDate.before(dates1[1])) {
                endDate = dates1[1];
            }
        }
    ...
    public MrpVer createMrpVer(String[] dpsVers, String[] mpsVers) {
        log.info("创建MRP版本");
        Date[] dates = checkVer(dpsVers, mpsVers);
        Date startDate = dates[0];
        Date endDate = dates[1];
        String plant;
        if(dpsVers != null && dpsVers.length >0) {
            plant = dpsService.getDpsVer(dpsVers[0]).getFab();
        } else {
            plant = mpsService.getMpsVer(mpsVers[0]).getFab();
        }
        String dpsVerStr = null;
        String mpsVerStr = null;
        if(dpsVers != null) {
            for(String ver : dpsVers) {
                if(StringUtils.isEmpty(dpsVerStr)) {
                    dpsVerStr = ver;
                } else {
                    dpsVerStr += "," + ver;
                }
        ...
        if(mpsVers != null) {
            for(String ver : mpsVers) {
                if(StringUtils.isEmpty(mpsVerStr)) {
                    mpsVerStr = ver;
                } else {
                    mpsVerStr += "," + ver;
                }
...
//        mrpVerRepository.saveAndFlush(mrpVer);
    ...
    public void computeDemand(MrpVer mrpVer) throws IOException, ClassNotFoundException {
        ...
        //MPS
        String[] mpsVers = mrpVer.splitDpsVer();
        if(mpsVers != null && mpsVers.length>0) {
    ...
    public void computeDemandProduct(DpsVer dpsVer, String product) throws IOException, ClassNotFoundException {
        ...
        for(DpsData dps : dpsList) {
            ...
            for(Demand demand : demandList) {
                ...
                if(StringUtils.equalsAnyIgnoreCase(plant, "CELL", "ARY")) {
                    demandQty = DoubleUtil.upPrecision(demandQty, 3);
                } else {
    ...
    public List<Demand> cellDemand(String plant, String outputName) {
        ...
        for(Bom bom : bomList) {
            if(bom.isEffectFlag()) {
                materialList.add(bom.getMaterial());
            }
        }
        //获取CELL BOM
        List<Map> bomMapList = cellBomService.getCellBom(materialList);
        List<Demand> demandList = new ArrayList<>();
        if(bomMapList == null || bomMapList.size()==0) return demandList;
        for(Map map : bomMapList) {
            Demand demand = new Demand();
            demand.setProduct(outputName);
            String material = (String)map.get( ("material").toUpperCase() );
            demand.setMaterial(material);
            //BOM用量
            demand.setUsageQty( ((BigDecimal) map.get( ("usageQty").toUpperCase() )).doubleValue() );
            //替代料比例
            demand.setSubstituteRate(materialSubstituteService.getMaterialSubstituteRate(plant, product, material));
            //切片数
    ...
    public List<MrpData> computeMrp(MrpVer mrpVer, MrpDataMaster mrpDataMaster, List<MrpData> mrpDataList) {
        ...
        for(int i=0; i<mrpDataList.size(); i++) {
            ...
            //ARY/CELL保留三位小数，LCM保留整数，向上取整
            if(StringUtils.equalsAnyIgnoreCase(mrpVer.getPlant(), "CELL", "ARY")) {
                lossQty = DoubleUtil.upPrecision(lossQty, 3);
            } else {
...
//        checkVer(dpsVers, null);
        MrpVer mrpVer = createMrpVer(dpsVers, mpsVers);
